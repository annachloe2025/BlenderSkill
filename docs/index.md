...
| 項目 | 内容 |
|---|---|
| **フェーズ** | Phase 3 マテリアル編（はじめての着色モデル）|
| **最終更新** | 2026-05-04 |
| **直近の動き** | Phase 2 完了（ブーリアン）/ **Principled BSDF で椅子に着色 + Cycles レンダー** |
| **次のアクション** | PBRテクスチャ（Polyhaven）→ ノードグラフ作成 → UVアンラップ |

## 🪑 直近の成果

![着色済みの椅子（Cycles）](images/chair_v2_material.png)

*Phase 3 入口で椅子に質感を付与（木の座面・金属の脚・クリームの床）。[スクリプト](snippets/material_basics.md) / [元の椅子](snippets/simple_chair.md)*
...
| 3 | マテリアル編（Principled BSDF・PBR・UV・ノード）| 🟢 着手中（着色済み椅子）|
